(function(angular, $) {
    ...
    angular.module('FileManager').controller('FileManagerCtrl', [
        ...
        $scope.initPath = $scope.config.initPath.split('/');
        $scope.initPath.shift();
        $scope.reverse = false;
        ...
        $scope.query = '';
        $scope.fileNavigator = new FileNavigator($scope.initPath);
        ...
        $scope.$on("setCurrentPath", function(event, data) {
            console.log("receive:" + data);
            $scope.fileNavigator.currentPath = data;
            $scope.fileNavigator.refresh();
        })
        $scope.$emit("handshake", "hello");
